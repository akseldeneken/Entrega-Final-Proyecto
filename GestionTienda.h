   ...
   void mostrarInventario(const vector<Producto>& Productos) {
    ...
    for (const auto& producto : Productos) {
        if (producto.getStock() < limiteStock) {
            ...
            archivo << "Unidades necesarias para alcanzar nivel arriba del límite seleccionado: " << limiteStock - producto.getStock() << endl;
    ...
    archivo << "Total de productos en alerta de 'Sold Out': " << productosEnAlerta << endl;
